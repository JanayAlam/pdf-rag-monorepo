...
export class PDFRAGAgent {
  private static instance: ReturnType<typeof createAgent> | null = null;

  static getInstance() {
    if (!PDFRAGAgent.instance) {
      const basicModel = new ChatOpenAI({
        model: "gpt-4.1-mini",
      });

      PDFRAGAgent.instance = createAgent({
        model: basicModel,
        tools: [],
...
DO NOT answer on your own. If the answer is not in the context, reply exactly: "I don't know about what the user is asking."`,
        responseFormat: z.object({
          message: z
            .string()
            .describe(
              "Markdown response based strictly on the provided context",
            ),
        }),
      });
    }

    return PDFRAGAgent.instance;
  }
}
